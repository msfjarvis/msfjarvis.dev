...
Images stored in `public/posts/[slug]/`. The `src` prop is an absolute path (`/posts/[slug]/image.jpg`). No Astro optimization pipeline — `public/` files are copied as-is per the Astro docs: _"always served or copied as-is, without transform or bundling"_. Passing a string path to `<Image>` produces an unoptimized plain `<img>`, so a plain `<figure>/<img>` is used directly.
