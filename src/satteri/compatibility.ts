...
function alertIcon(type: AlertType): Emphasis {
  return {
    type: "emphasis",
    data: {
      hName: "svg",
    ...
    children: [
      {
        ...
        data: {
          hName: "path",
          hProperties: { d: alertIconPaths[type] },
        },
        children: [],
      },
    ],
...
export const githubAlerts = defineMdastPlugin({
  ...
  blockquote(node) {
    ...
    for (const child of replacement.children) {
      if (!seekingMarker || child.type !== "paragraph") continue;
      ...
      seekingMarker = false;
      alertType = match[1]?.toLocaleLowerCase() as AlertType;

      if (text.includes("\n")) {
        child.children[0] = {
          type: "text",
          value: text.replace(alertPattern, "").replace(/^\n+/, ""),
        };
      } else {
...
export const remarkSmartypantsCompatibility = defineMdastPlugin({
  name: "remark-smartypants-compatibility",
...
export const legacyTableAlignment = defineHastPlugin({
  ...
  element: {
    ...
    visit(node, context) {
      ...
      const match = /^text-align:\s*(left|right|center)\s*;?$/.exec(style);
      if (!match) return;

      context.setProperty(node, "align", match[1]);
