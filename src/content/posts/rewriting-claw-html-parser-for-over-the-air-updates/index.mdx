...
> To skip right to the juicy parts, go to [The great Zipline migration](#the-great-zipline-migration) section.
...
> [!WARNING]
> There may be overlap between the _right_ way to use Zipline and _my_ way to use Zipline, but expect that number to trend towards zero. I've solved a lot of problems by bashing my head against a keyboard and or an LLM, which aren't necessarily the best ways to go about it.
...
The deserialization code will look pretty self-explanatory if you were able to follow the serializer. We define an `index` variable at value 0, read one byte off the input string at that index to calculate how far ahead we need to go, and slice off that much after the `:` for our first variable's value. This is repeated until the full string is traversed, after which we assemble the actual Kotlin object.
