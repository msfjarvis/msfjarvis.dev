...
import grafana_profile from '../../content/images/posts/creating-private-services-on-nixos-using-tailscale-and-caddy/grafana-profile.webp';
...
<Figure src={grafana_profile} alt="The profile page on Grafana, showing the name, email and username fields with an additional 'synced via auth proxy' label next to them. The email and username have been blurred out to redact them for privacy reasons" title="Grafana profile showing that user identification is being done via the Tailscale proxy" />
