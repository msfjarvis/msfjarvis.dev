...
import obsidian_weeknotes from "./obsidian-weeknotes.webp";

Following up on my [Mastodon](https://masto-archive.msfjarvis.dev/@msfjarvis/115717823447859331) post about wanting to start doing week notes, inspired by [Ankur](https://ankursethi.com/) and [Abhinav](https://abhinavsarkar.net/notes/tags/weeknotes/), here's the quick rundown of how I supplemented my existing Obsidian journal to facilitate the new endeavor.
...
<Figure
  src={obsidian_weeknotes}
  alt="An Obsidian page with the title 2025-W50 and listing two journal entries from today: Me writing about my Droidcon India 2025 experience and a mention of me finishing my second run of the game Dispatch."
  title="Week 50 roundup with entries from Sunday"
/>
...
(() => {
  ...
  for (const p of pages) {
    const bullets = (p.file.lists ?? []).filter((li) =>
      (li.text ?? "").includes(TAG),
    );
